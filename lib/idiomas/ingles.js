...
const smsUnreg = () => { return `\`\`\`¡¡YOU NEED TO BE REGISTERED USE CMD .verify yourname.yourage\`\`\`` }
...
const smsJBConexionTrue2 = () => { return `${lenguajeGB['smsAvisoEG']()}🟢 *SUCCESSFUL CONNECTION!!! YOU CAN CONNECT USING YOUR (ID) OR USE:*`}
const smsJBConexionTrue3 = () => { return ` *OR YOU CAN USE YOUR (ID)*`}
const smsJBCargando = (usedPrefix) => { return `${lenguajeGB['smsAvisoIIG']()}⚪ *IS CONNECTED!! PLEASE WAIT MESSAGES ARE LOADING...*\n\n♻️ *AVAILABLE OPTIONS:*\n*» ${usedPrefix}pausesb _(Stop Sub Bot Feature)_*\n*» ${usedPrefix}delsession _(Delete all traces of Sub Bot)_*\n*» ${usedPrefix}jadibot _(Get new QR code to be Sub Bot)_*`}
...
//config-on y off.js
const smsConfi1bot = () => { return `⚙️ *SETTING* ⚙️`}
